...
def invoice_pdf(inv, *, demo=False, paid_override=None, status_override=None):
    ...
    if demo:story.extend([p('DEMOSTRACIÓN · SIN VALIDEZ FISCAL','Heading2'),p('Datos ficticios. No acredita una venta ni un pago real.','SmallNexo'),Spacer(1,16)])
    ...
    paid=invoice_paid(inv) if paid_override is None else paid_override;status=invoice_status(inv) if status_override is None else status_override;remaining=0 if status in ['credit','credited'] else inv.document.total-paid
