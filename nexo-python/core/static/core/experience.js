...
function accountModal(){openModal('Mi cuenta',`<div class="account-info"><span class="avatar">${esc(state.user.name.slice(0,2))}</span><div><h3>${esc(state.user.name)}</h3><p>${roles[state.data.role]} · ${esc(state.data.business.name)}</p></div></div><div class="stack-buttons">${button(DEMO?'Acerca de mi prueba':'Cambiar contraseña','password','','secondary','shield')}${admin()&&!DEMO?button('Crear otro negocio','business','','secondary','plus'):''}<button class="button secondary danger" data-action="logout">Cerrar sesión</button></div>`)}
function helpModal(){
